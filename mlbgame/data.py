...
def get_important_dates(year):
    try:
        return gethtml(IMPORTANT_DATES.format(year))
    except HTTPError:
...
def get_scoreboard(year, month, day):
    ...
    try:
        data = gethtml(BASE_URL.format(year, month, day) + 'scoreboard.xml')
    except HTTPError:
...
def get_box_score(game_id):
    ...
    try:
        return gethtml(GAME_URL.format(year, month, day, game_id,
                                       'boxscore.xml'))
    except HTTPError:
...
def get_roster(team_id):
    ...
    try:
        return gethtml(ROSTER_URL.format(team_id))
    except HTTPError:
...
def get_standings(date):
    ...
    try:
        return gethtml(STANDINGS_URL.format(date.year,
                                            date.strftime('%Y/%m/%d')))
    except HTTPError:
...
def gethtml(url, user_agent='tblade' , num_retries = 5):
    ...
    if url_html.status == 200:
        return url_html
    else:
        print('Download error:', url_html)
        if num_retries > 0:
            if hasattr(url_html, 'status') and 500 <= url_html.status < 600:
